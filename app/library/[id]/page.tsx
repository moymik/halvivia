...
async function BookPageContent({ params }: BookPageProps) {
  ...
  return (
    <main className="bg-bg-base text-text-primary">
      <section className="mx-auto flex w-full max-w-[1100px] flex-col gap-6 px-4 py-8 md:px-8 lg:py-10">
        <Link
          href={ROUTES.LIBRARY}
        ...
        <div className="grid gap-6 md:grid-cols-[180px_minmax(0,1fr)] lg:grid-cols-[220px_minmax(0,1fr)]">
          <div className="relative aspect-[104/171] w-full max-w-[220px] overflow-hidden bg-neutral-200">
            {book.thumbnailUrl ? (
              <Image
          ...
          <div className="flex max-w-2xl flex-col gap-4">
            <div>
              ...
              {book.authors.length > 0 && (
                <p className="text-text-secondary mt-2 text-lg">{book.authors.join(', ')}</p>
              )}
            </div>
